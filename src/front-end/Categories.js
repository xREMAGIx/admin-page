...
import CategoryEditModal from "./CategoriesEdit";
...
const EnhancedTableToolbar = (props) => {
  ...
  const onDelete = (id) => {
    dispatch(categoryActions.delete(id));
  };
      ...
      >
        ...
        {numSelected > 0 ? (
          <Grid container direction="row" justify="flex-end" spacing={1}>
            {numSelected < 2 ? (
              <Grid item>
                <CategoryEditModal id={selectedIndex[0]} />
              </Grid>
            ) : null}
            <Grid item>
...
export default function Categories() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = categories.items.map((n) => n._id);
              ...
              >
                {!categories.items ? (
                  ...
                  <EnhancedTableHead
                    ...
                    rowCount={categories.items.length}
                  />
                )}
                {!categories.items ? (
